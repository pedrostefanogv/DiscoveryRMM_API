...
public sealed class RemoteDebugSessionManager : IRemoteDebugSessionManager
{
        ...
        int? ttlMinutes)
    {
        ...
        var normalizedTransport = NormalizeTransport(preferredTransport);
        ...
        var state = new RemoteDebugSessionState
        {
            ...
            NatsLegacySubject = NatsSubjectBuilder.AgentLegacySubject(agentId, "remote-debug.log"),
            NatsTenantSubject = NatsSubjectBuilder.AgentSubject(clientId, siteId, agentId, "remote-debug.log")
        };
    ...
    private static string NormalizeTransport(string? preferredTransport)
    {
        if (string.IsNullOrWhiteSpace(preferredTransport))
            return "signalr";

        var normalized = preferredTransport.Trim().ToLowerInvariant();
        return normalized is "signalr" or "nats-wss" or "nats"
            ? normalized
            : "signalr";
    }
}
