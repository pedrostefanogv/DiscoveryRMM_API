...
public class AgentsController : ControllerBase
{
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        ...
        if (!string.IsNullOrWhiteSpace(agent?.MeshCentralNodeId))
        {
            try
            {
                await _meshCentralApiService.RemoveDeviceAsync(agent.MeshCentralNodeId, HttpContext.RequestAborted);
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "MeshCentral cleanup failed for agent {AgentId} and node {MeshNodeId}", id, agent.MeshCentralNodeId);
            }
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var values = await _customFieldService.GetValuesAsync(CustomFieldScopeType.Agent, id, includeSecrets, cancellationToken);
        return Ok(values);
    }
    ...
    public async Task<IActionResult> UpsertCustomFieldValue(
        Guid id,
        Guid definitionId,
        [FromBody] UpsertAgentCustomFieldValueRequest request,
        CancellationToken cancellationToken = default)
    {
        var agent = await _agentRepo.GetByIdAsync(id);
        if (agent is null)
    ...
    public async Task<IActionResult> GetHardware(Guid id)
    {
        ...
        return Ok(new
        {
            ...
            OpenSockets = payload.OpenSockets
        });
    }
    ...
    public async Task<IActionResult> StartRemoteDebug(Guid id, [FromBody] StartRemoteDebugRequest? request)
    {
        ...
        return Ok(new RemoteDebugStartResponse(
            ...
            session.FallbackTransport,
            session.NatsSubject,
            session.SignalRMethod));
    }
    ...
    public async Task<IActionResult> RunAutomationTaskNow(Guid id, Guid taskId, CancellationToken cancellationToken = default)
    {
        ...
        var created = await _commandDispatcher.DispatchAsync(command, cancellationToken);
    ...
    private sealed record AgentHardwareCachePayload(
        ...
        IReadOnlyList<ListeningPortInfo> ListeningPorts,
        IReadOnlyList<OpenSocketInfo> OpenSockets);
...
public record HardwareComponentsPayload(
    ...
    List<ListeningPortInfo>? ListeningPorts,
    List<OpenSocketInfo>? OpenSockets);
