...
public record CreateAgentRequest(Guid SiteId, string Hostname, string? DisplayName, string? OperatingSystem, string? OsVersion, string? AgentVersion, string? CommitHash);
...
public record HardwareReportRequest(string? Hostname, string? DisplayName, string? MeshCentralNodeId, AgentStatus? Status, string? OperatingSystem, string? OsVersion, string? AgentVersion, string? CommitHash, string? LastIpAddress, string? MacAddress, AgentHardwareInfo? Hardware, HardwareComponentsPayload? Components, JsonElement? InventoryRaw, string? InventorySchemaVersion, DateTime? InventoryCollectedAt, int? MachineScore);
