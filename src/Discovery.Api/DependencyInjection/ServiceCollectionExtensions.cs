...
{
    /// <summary>
    /// Registers all services from Discovery.Infrastructure that implement a single interface from Discovery.Core.Interfaces.
    /// This reduces the need to manually register every service/repository one-by-one.
    /// </summary>
    public static IReadOnlyList<AutoServiceRegistration> AddDiscoveryAutoRegisteredServices(this IServiceCollection services)
    {
        // Scan the infrastructure assembly for concrete implementations
        var infrastructureAssembly = typeof(Discovery.Infrastructure.Repositories.ClientRepository).Assembly;
        ...
        var coreInterfaces = coreInterfaceAssembly.GetTypes()
            .Where(t => t.IsInterface && t.IsPublic && t.Namespace != null && t.Namespace.StartsWith("Discovery.Core.Interfaces", StringComparison.Ordinal))
            .ToArray();
