...
public class ConfigurationResolver : IConfigurationResolver
{
    ...
    {
        var policy = !string.IsNullOrWhiteSpace(siteJson)
            ? DeserializeOrDefault<AgentUpdatePolicy>(siteJson)
            : !string.IsNullOrWhiteSpace(clientJson)
                ? DeserializeOrDefault<AgentUpdatePolicy>(clientJson)
                : DeserializeOrDefault<AgentUpdatePolicy>(serverJson);
