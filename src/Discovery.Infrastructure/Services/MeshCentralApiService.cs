...
public class MeshCentralApiService : IMeshCentralApiService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _ = _meshCentralConfigService.GetPublicBaseUrl();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await ExecuteWithSocketAsync(async (ws, ct) =>
        {
            ...
            var responseId = "Discovery.addmeshuser." + Guid.NewGuid().ToString("N");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await ExecuteWithSocketAsync(async (ws, ct) =>
        {
            ...
            var responseId = "Discovery.removemeshuser." + Guid.NewGuid().ToString("N");
            await SendAsync(ws, new
            {
    ...
    public async Task<MeshCentralDeviceAclSyncResult> EnsureUserOnDeviceAsync(
        string meshUserId,
        string meshNodeId,
        int rights,
        CancellationToken cancellationToken = default)
    {
        ...
        var normalizedNodeId = NormalizeNodeId(meshNodeId);

        await ExecuteWithSocketAsync(async (ws, ct) =>
        {
            var responseId = "Discovery.adddeviceuser." + Guid.NewGuid().ToString("N");
            await SendAsync(ws, new
            {
                action = "adddeviceuser",
                nodeid = normalizedNodeId,
                userids = new[] { meshUserId },
                rights,
                responseid = responseId
            }, ct);
            ...
            var result = reply.GetPropertyOrDefault("result");
            if (!IsOperationOk(result))
                throw new InvalidOperationException("MeshCentral adddeviceuser failed: " + result);

            return true;
        }, cancellationToken);

        return new MeshCentralDeviceAclSyncResult
        {
            UserId = meshUserId,
            NodeId = normalizedNodeId,
            Granted = true,
    ...
    public async Task<MeshCentralDeviceAclSyncResult> RemoveUserFromDeviceAsync(
        string meshUserId,
        string meshNodeId,
        CancellationToken cancellationToken = default)
    {
        ...
        await ExecuteWithSocketAsync(async (ws, ct) =>
        {
            var responseId = "Discovery.removedeviceuser." + Guid.NewGuid().ToString("N");
            await SendAsync(ws, new
            {
                action = "adddeviceuser",
                nodeid = normalizedNodeId,
            ...
            var result = reply.GetPropertyOrDefault("result");
            if (!IsOperationOk(result) && !IsNotFoundResult(result))
                throw new InvalidOperationException("MeshCentral remove device user failed: " + result);
    ...
    public async Task<IReadOnlyCollection<MeshCentralNodeRef>> ListNodesAsync(
        string? meshId = null,
        CancellationToken cancellationToken = default)
    {
        ValidateConnectionSettings();

        return await ExecuteWithSocketAsync(
            (ws, ct) => ListNodesAsync(ws, meshId, ct),
            cancellationToken);
    }
    ...
    public async Task DeleteUserAsync(string meshUserId, CancellationToken cancellationToken = default)
    {
        ...
        await ExecuteWithSocketAsync(async (ws, ct) =>
        {
            var responseId = "Discovery.deleteuser." + Guid.NewGuid().ToString("N");
            await SendAsync(ws, new
            {
                action = "deleteuser",
                userid = meshUserId,
    ...
    public async Task<MeshCentralHealthCheckResult> RunHealthCheckAsync(CancellationToken cancellationToken = default)
    {
        ValidateConnectionSettings();

        var publicBaseUrl = _meshCentralConfigService.GetPublicBaseUrl();
        var administrativeBaseUrl = _meshCentralConfigService.GetAdministrativeBaseUrl();
        var technicalUsername = _meshCentralConfigService.GetTechnicalUsername();

        return await ExecuteWithSocketAsync(async (ws, ct) =>
        {
            var users = await ListUsersAsync(ws, ct);
            var meshes = await ListMeshesAsync(ws, ct);

            return new MeshCentralHealthCheckResult
            {
                ControlSocketConnected = true,
                PublicBaseUrl = publicBaseUrl,
                AdministrativeBaseUrl = administrativeBaseUrl,
    ...
    public async Task RemoveDeviceAsync(string meshNodeId, CancellationToken cancellationToken = default)
    {
        ValidateConnectionSettings();
        ...
        await ExecuteWithSocketAsync(async (ws, ct) =>
        {
            var responseId = "Discovery.removedevice." + Guid.NewGuid().ToString("N");
            await SendAsync(ws, new
            {
                action = "removedevices",
                nodeids = new[] { normalizedNodeId },
                responseid = responseId
            }, ct);
            ...
            var result = reply.GetPropertyOrDefault("result");
            if (!IsOperationOk(result))
                throw new InvalidOperationException("MeshCentral removedevices failed: " + result);
    ...
    private void ValidateConnectionSettings()
    {
        ...
        _ = _meshCentralConfigService.GetAdministrativeBaseUrl();
        _ = _meshCentralTokenService.GenerateControlAuthToken(_meshCentralConfigService.GetTechnicalUsername());
    }
    ...
    private async Task<T> ExecuteWithSocketAsync<T>(Func<ClientWebSocket, CancellationToken, Task<T>> callback, CancellationToken cancellationToken)
    {
        ...
        var authToken = _meshCentralTokenService.GenerateControlAuthToken(_meshCentralConfigService.GetTechnicalUsername());
        var wsUri = _meshCentralConfigService.BuildControlWebSocketUri(authToken);
    ...
    {
        var responseId = "Discovery.users." + Guid.NewGuid().ToString("N");
    ...
    private async Task<IReadOnlyCollection<MeshCentralMeshRef>> ListMeshesAsync(ClientWebSocket ws, CancellationToken ct)
    {
        var responseId = "Discovery.meshes." + Guid.NewGuid().ToString("N");
        ...
        var meshes = new List<MeshCentralMeshRef>();
        foreach (var mesh in meshesNode.EnumerateArray())
        {
            ...
            meshes.Add(new MeshCentralMeshRef(id, userLinks));
        }

        return meshes;
    }

    private async Task<IReadOnlyCollection<MeshCentralNodeRef>> ListNodesAsync(
        ClientWebSocket ws,
        string? meshId,
        CancellationToken ct)
    {
        var responseId = "Discovery.nodes." + Guid.NewGuid().ToString("N");
        if (string.IsNullOrWhiteSpace(meshId))
        {
            await SendAsync(ws, new { action = "nodes", responseid = responseId }, ct);
        }
        else
        {
            await SendAsync(ws, new { action = "nodes", meshid = meshId, responseid = responseId }, ct);
        }
        ...
        if (!reply.TryGetProperty("nodes", out var nodesNode))
            return [];

        var nodes = new List<MeshCentralNodeRef>();
        if (nodesNode.ValueKind == JsonValueKind.Array)
        {
            foreach (var node in nodesNode.EnumerateArray())
            {
                var mapped = ParseNode(node, meshId);
                if (mapped is not null)
                    nodes.Add(mapped);
            }
        ...
        if (nodesNode.ValueKind != JsonValueKind.Object)
            return nodes;

        foreach (var meshEntry in nodesNode.EnumerateObject())
        {
            if (meshEntry.Value.ValueKind == JsonValueKind.Array)
            {
                foreach (var node in meshEntry.Value.EnumerateArray())
                {
                    var mapped = ParseNode(node, meshEntry.Name);
                    if (mapped is not null)
                        nodes.Add(mapped);
                }
            ...
            var single = ParseNode(meshEntry.Value, meshEntry.Name);
    ...
    {
        var responseId = "Discovery.adduser." + Guid.NewGuid().ToString("N");
        await SendAsync(ws, new
        {
            action = "adduser",
            username,
    ...
    {
        var responseId = "Discovery.edituser." + Guid.NewGuid().ToString("N");
        await SendAsync(ws, new
        {
            action = "edituser",
            userid = userId,
    ...
    private async Task<MeshCentralMeshRef> GetMeshByIdAsync(ClientWebSocket ws, string meshId, CancellationToken ct)
    {
        ...
        {
            if (!string.Equals(mesh.MeshId, meshId, StringComparison.OrdinalIgnoreCase))
                continue;
    ...
    private static MeshCentralNodeRef? ParseNode(JsonElement element, string? fallbackMeshId = null)
    {
        var nodeId = element.GetPropertyOrDefault("_id")
                     ?? element.GetPropertyOrDefault("id")
                     ?? element.GetPropertyOrDefault("nodeid");
        var meshId = element.GetPropertyOrDefault("meshid") ?? fallbackMeshId;

        if (string.IsNullOrWhiteSpace(nodeId) || string.IsNullOrWhiteSpace(meshId))
            return null;

        return new MeshCentralNodeRef
        {
            NodeId = nodeId,
            MeshId = meshId,
            Name = element.GetPropertyOrDefault("name"),
            Hostname = element.GetPropertyOrDefault("host") ?? element.GetPropertyOrDefault("hostname")
        };
    }
    ...
    private static bool IsNotFoundResult(string? result)
    {
        if (string.IsNullOrWhiteSpace(result))
            return false;

        return result.Contains("not found", StringComparison.OrdinalIgnoreCase)
               || result.Contains("unknown device", StringComparison.OrdinalIgnoreCase)
               || result.Contains("unknown user", StringComparison.OrdinalIgnoreCase);
    }
    ...
    private static string NormalizeNodeId(string meshNodeId)
    {
        ...
        if (string.IsNullOrWhiteSpace(normalized) || !normalized.StartsWith("node/", StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException("MeshCentral node id is invalid.");
    ...
    {
        var listResponseId = "Discovery.meshes." + Guid.NewGuid().ToString("N");
        ...
        var createResponseId = "Discovery.createmesh." + Guid.NewGuid().ToString("N");
        await SendAsync(ws, new
        {
            action = "createmesh",
            meshname = groupName,
    ...
    private static string BuildGroupName(Client client, Site site)
    {
        ...
        return $"discovery-{clientPart}-{sitePart}";
    }
