...
public class AgentPackageService : IAgentPackageService
{
    ...
    /// <inheritdoc />
    public Task<string> GetAgentCommitHashAsync(CancellationToken cancellationToken = default)
    {
        var projectPath = GetAgentPackageSetting("DiscoveryProjectPath")
            ?? (OperatingSystem.IsWindows() ? @"C:\Projetos\Discovery" : "/opt/discovery-agent-src");

        // Level 1: try agent-version.json (most reliable — generated by the agent build script)
        var (_, commitHash) = ReadAgentVersionJson(projectPath);
        if (!string.IsNullOrWhiteSpace(commitHash) && !string.Equals(commitHash, "unknown", StringComparison.OrdinalIgnoreCase))
        {
            _logger.LogInformation("Agent commit hash resolved from agent-version.json: {CommitHash}", commitHash);
            return Task.FromResult(commitHash);
        }

        // Level 2: git rev-parse in the configured project path
        if (Directory.Exists(projectPath))
        {
            try
            {
                var psi = new ProcessStartInfo("git", $"-C \"{projectPath}\" rev-parse --short=8 HEAD")
                {
                    RedirectStandardOutput = true,
                    UseShellExecute = false,
                    CreateNoWindow = true
                ...
                if (proc is not null)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(output) && proc.ExitCode == 0)
                    {
                        _logger.LogInformation("Agent commit hash resolved from git: {CommitHash}", output);
            ...
            {
                _logger.LogDebug(ex, "Failed to resolve agent commit hash via git in {ProjectPath}", projectPath);
            }
        }

        // Level 3: fallback
        _logger.LogWarning("Could not resolve agent commit hash; using 'unknown'");
        return Task.FromResult("unknown");
    }

    /// <inheritdoc />
    public Task<string> GetAgentSourceVersionAsync(CancellationToken cancellationToken = default)
    {
        var projectPath = GetAgentPackageSetting("DiscoveryProjectPath")
            ?? (OperatingSystem.IsWindows() ? @"C:\Projetos\Discovery" : "/opt/discovery-agent-src");

        // Level 1: wails.json productVersion (authoritative source)
        var wailsVersion = DetectAgentVersion(projectPath);
        if (!string.IsNullOrWhiteSpace(wailsVersion))
        {
            _logger.LogInformation("Agent version resolved from wails.json: {Version}", wailsVersion);
            return Task.FromResult(wailsVersion);
        }

        // Level 2: agent-version.json (generated by build script)
        var (version, _) = ReadAgentVersionJson(projectPath);
        if (!string.IsNullOrWhiteSpace(version) && !string.Equals(version, "unknown", StringComparison.OrdinalIgnoreCase))
        {
            _logger.LogInformation("Agent version resolved from agent-version.json: {Version}", version);
            return Task.FromResult(version);
        }

        // Level 3: fallback
        _logger.LogWarning("Could not resolve agent source version; using 'unknown'");
        return Task.FromResult("unknown");
    }
