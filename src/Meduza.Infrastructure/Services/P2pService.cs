...
public class P2pService : IP2pService
{
    ...
    public async Task<int> CheckTelemetryRateLimitAsync(Guid agentId, CancellationToken ct = default)
    {
        ...
        var raw = await _redis.GetAsync(key);
        ...
        if (count >= maxRequests)
            return windowSeconds; // retryAfterSeconds — simplificado

        await _redis.SetAsync(key, (count + 1).ToString(), windowSeconds);
        return 0;
    }
        ...
        Guid resolvedSiteId)
    {
        ...
        {
            errors.Add(new P2pErrorDetail { Field = "metrics", Code = "INVALID_RFC3339", Message = "metrics é obrigatório" });
        }
        else
        {
        ...
        {
            errors.Add(new P2pErrorDetail { Field = "currentSeedPlan", Code = "METRIC_NEGATIVE", Message = "currentSeedPlan é obrigatório" });
        }
        else
        {
