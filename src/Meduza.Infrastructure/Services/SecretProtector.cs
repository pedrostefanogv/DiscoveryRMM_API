...
public sealed class SecretProtector : ISecretProtector
{
    ...
    public string Protect(string plaintext)
    {
        ...
        var tag = new byte[16];
        var aad = Encoding.UTF8.GetBytes($"meduza:{_keyId}");
    ...
    public string Unprotect(string protectedValue)
    {
        ...
        var aad = Encoding.UTF8.GetBytes($"meduza:{keyId}");
    ...
    private static byte[]? ResolveMasterKey(string? configuredBase64)
    {
        var candidate = string.IsNullOrWhiteSpace(configuredBase64)
            ? Environment.GetEnvironmentVariable("MEDUZA_ENCRYPTION_KEY_BASE64")
            : configuredBase64;
