...
public class MeshCentralOptions
{
    ...
    /// <summary>
    /// URL interna do MeshCentral usada pelo backend para control.ashx.
    ...
    /// <summary>
    /// Usuario tecnico usado pelo backend para operacoes administrativas.
    /// Sempre resolvido em lowercase.
    /// </summary>
    public string TechnicalUsername { get; set; } = "admin";

    /// <summary>
    /// Compatibilidade temporaria para ambientes que ainda exigem key= na URL do control.ashx.
    /// O default seguro e false.
    /// </summary>
    public bool AdministrativeIncludeKeyInQuery { get; set; } = false;
    ...
    /// <summary>
    /// Direitos administrativos aplicados ao vinculo do usuario no mesh/site.
    /// O default seguro e 0, deixando a autorizacao efetiva no nivel de device ACL.
    /// </summary>
    public int IdentitySyncMeshMembershipRights { get; set; } = 0;

    /// <summary>
    /// Quando true, remove ACLs de device que nao fazem mais parte do escopo desejado.
    /// O default seguro preserva grants manuais ate que a operacao seja validada.
    /// </summary>
    public bool IdentitySyncDeviceAclRevocationEnabled { get; set; } = false;
