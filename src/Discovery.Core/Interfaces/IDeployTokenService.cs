...
public interface IDeployTokenService
{
    ...
    /// <summary>
    /// Validates raw token without consuming it and returns only active tokens
    /// (not revoked, not expired and not depleted).
    /// </summary>
    Task<DeployToken?> GetValidatedAsync(string rawToken);
    /// <summary>
