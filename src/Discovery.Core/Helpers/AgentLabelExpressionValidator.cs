...
public static class AgentLabelExpressionValidator
{
    ...
    public static IReadOnlyList<string> Validate(AgentLabelRuleExpressionNodeDto expression)
    {
        var errors = new List<string>();
        var nodeCount = 0;
        ValidateNode(expression, 1, ref nodeCount, errors, "root");
        return errors;
    }
        ...
        string path)
    {
        ...
        if (node.NodeType == AgentLabelNodeType.Group)
        {
            ...
            {
                ValidateNode(node.Children[i], depth + 1, ref nodeCount, errors, $"{path}.children[{i}]");
            }
        ...
        ValidateCondition(node.Field.Value, node.Operator.Value, node.Value, errors, path);
    }
