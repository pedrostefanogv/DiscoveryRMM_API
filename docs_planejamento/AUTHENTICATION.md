...
| MFA OTP assertion | `POST /api/auth/mfa/otp/complete` | OTP/TOTP assertion flow for accounts configured to require OTP during login. |
| MFA registration | `POST /api/mfa/fido2/register/begin`, `POST /api/mfa/fido2/register/complete`, `POST /api/mfa/totp/register/begin`, `POST /api/mfa/totp/register/complete` | Accepts onboarding token or full access token. |
...
- FIDO2 remains the primary MFA recommendation, and OTP/TOTP is also available for registration and login assertion flows.
...
- Login/refresh and MFA assertion responses now expose stable auth error codes in the payload (for example `invalid_credentials`, `refresh_invalid`, `mfa_invalid`, `otp_invalid`) so clients can map failures consistently.
