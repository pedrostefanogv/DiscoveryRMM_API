...
install_acme_sh() {
  ...
  install -d -m 750 -o root -g root "$(dirname "$LETSENCRYPT_ACME_SH_DIR")"
  ...
  if ! git clone --depth 1 --branch "${ACME_SH_VERSION:-v3.1.1}" \
      https://github.com/acmesh-official/acme.sh.git "$LETSENCRYPT_ACME_SH_DIR" 2>/dev/null; then
...
register_account() {
  ...
  install -d -m 700 -o root -g root "$LETSENCRYPT_ACME_HOME"
...
install_certificate() {
  ...
  if ! openssl x509 -in "$LETSENCRYPT_CERT_FULLCHAIN_PATH" -noout -checkend 0 >/dev/null 2>&1; then
    fail "Certificado instalado invalido ou expirado ($LETSENCRYPT_CERT_FULLCHAIN_PATH)."
  fi
  if ! openssl x509 -in "$LETSENCRYPT_CERT_FULLCHAIN_PATH" -noout -checkhost "$LETSENCRYPT_CERT_DOMAIN" >/dev/null 2>&1; then
    fail "Certificado instalado nao corresponde ao dominio $LETSENCRYPT_CERT_DOMAIN."
  fi
}
...
issue_or_renew() {
  ...
  exec 9>/run/lock/discovery-acme-letsencrypt.lock
  if ! flock -n 9; then
    fail "Outra emissao/renovacao Let's Encrypt em andamento (lock em /run/lock)."
  fi
  ...
  if [[ "$command_action" == "--renew" && -z "${LETSENCRYPT_DNS_AUTOMATION_HOOK:-}" && ! -t 0 ]]; then
    fail "Renovacao Let's Encrypt usa DNS manual e nao ha terminal interativo. Execute este script manualmente ou configure LETSENCRYPT_DNS_AUTOMATION_HOOK."
  fi
