...
NATS_JS_FANOUT_STREAM_SUBJECTS=${NATS_JS_FANOUT_STREAM_SUBJECTS:-tenant.*.site.*.agents.command,tenant.*.agents.command}
...
update_nats_environment_file() {
  ...
  sudo awk '
    ...
    !/^NATS_JS_MAX_FILE_STORE=/ &&
    !/^NATS_JS_FANOUT_STREAM_ENABLED=/ &&
    !/^NATS_JS_FANOUT_STREAM_NAME=/ &&
    !/^NATS_JS_FANOUT_STREAM_SUBJECTS=/ &&
